                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {services.map((service, index) => (
                        <Card key={index} className="glass-card border-none group relative overflow-hidden">
                            ...
                            <CardHeader>
                                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center mb-4 ring-1 ring-white/20 group-hover:ring-primary/50 transition-all backdrop-blur-md">
                                    <service.icon className="w-6 h-6 text-primary group-hover:text-white transition-colors" />
                                </div>
                                <CardTitle className="text-xl relative z-10">{service.title}</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <CardDescription className="text-base relative z-10 text-muted-foreground group-hover:text-foreground/90 transition-colors">
                                    {service.description}
                                </CardDescription>
                            </CardContent>
                        </Card>
